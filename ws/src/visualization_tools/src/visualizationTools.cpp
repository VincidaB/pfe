...
void laserCloudHandler(const sensor_msgs::msg::PointCloud2::ConstSharedPtr laserCloudIn)
{
  ...
  *exploredAreaCloud += *laserCloud;
  ...
  if (exploredAreaDisplayCount >= 10 * exploredAreaDisplayInterval) {
    ...
    exploredArea2.header.frame_id = "camera_init";
...
int main(int argc, char** argv)
{
  ...
  mapFile.replace(mapFile.find("/install/"), 8, "");
  metricFile.replace(metricFile.find("/install/"), 8, "");
  trajFile.replace(trajFile.find("/install/"), 8, "");
  pcdFile.replace(pcdFile.find("/install/"), 8, "");
