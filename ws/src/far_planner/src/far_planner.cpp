...
void FARMaster::OdomCallBack(const nav_msgs::msg::Odometry::SharedPtr msg) {
  ...
  if (!FARUtil::IsSameFrameID(odom_frame, master_params_.world_frame)) {
    std::cout << "odom frame problem !!!" << std::endl;
    if (FARUtil::IsDebug) RCLCPP_WARN(nh_->get_logger(), "FARMaster: odom frame does NOT match with world frame!");
  ...
  //RCLCPP_INFO(nh_->get_logger(), "robot position: %f, %f, %f", robot_pos_.x, robot_pos_.y, robot_pos_.z);
...
void FARMaster::PrcocessCloud(const sensor_msgs::msg::PointCloud2::SharedPtr pc, const PointCloudPtr& cloudOut) 
{
  ...
  try {
    FARUtil::FilterCloud(cloudOut, master_params_.voxel_dim);
      ...
      std::cerr << "Error in Filter cloud: " << e.what() << std::endl;
      // Handle the exception, e.g., by setting a default value or skipping the operation
  } 
// transform cloud frame
  ...
  if (!FARUtil::IsSameFrameID(cloud_frame, master_params_.world_frame)) {
    ...
    if (FARUtil::IsDebug) std::cout <<"FARMaster: cloud frame does NOT match with world frame!" << std::endl;
    try
    {
