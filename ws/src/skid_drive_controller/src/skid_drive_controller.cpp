...
#include "skid_drive_controller/skid_drive_controller.hpp"
...
SkidDriveController::SkidDriveController() : controller_interface::ControllerInterface() {}

const char * SkidDriveController::feedback_type() const
{
  return params_.position_feedback ? HW_IF_POSITION : HW_IF_VELOCITY;
}

controller_interface::CallbackReturn SkidDriveController::on_init()
{
  try
  {
...
InterfaceConfiguration SkidDriveController::command_interface_configuration() const
{
...
InterfaceConfiguration SkidDriveController::state_interface_configuration() const
{
  std::vector<std::string> conf_names;
  for (const auto & joint_name : params_.left_wheel_names)
  {
...
controller_interface::return_type SkidDriveController::update(
  const rclcpp::Time & time, const rclcpp::Duration & period)
{
  ...
  if (age_of_last_command > cmd_vel_timeout_)
  {
    ...
    measured_odom_twist_.linear.x = 0.0;
  }
...
controller_interface::CallbackReturn SkidDriveController::on_configure(
  const rclcpp_lifecycle::State &)
{
  ...
  twist_subscriber_ = get_node()->create_subscription<geometry_msgs::msg::Twist>(
    ...
    [this](const std::shared_ptr<geometry_msgs::msg::Twist> msg) -> void
    {
      ...
      {
        RCLCPP_WARN(
          get_node()->get_logger(), "Can't accept new twist. subscriber is inactive");
        return;
      }
      RCLCPP_INFO(get_node()->get_logger(), "message received : %f", msg->linear.x);

      measured_odom_twist_.linear.x = msg->linear.x;
    }
  );
  ...
  */
  ...
  pid.initPid(1.0, 0.2, 0.1, 0.3, -0.3);
  
  previous_update_timestamp_ = get_node()->get_clock()->now();
  return controller_interface::CallbackReturn::SUCCESS;
}

controller_interface::CallbackReturn SkidDriveController::on_activate(
  const rclcpp_lifecycle::State &)
{
...
controller_interface::CallbackReturn SkidDriveController::on_deactivate(
  const rclcpp_lifecycle::State &)
{
...
controller_interface::CallbackReturn SkidDriveController::on_cleanup(
  const rclcpp_lifecycle::State &)
{
  if (!reset())
  {
...
controller_interface::CallbackReturn SkidDriveController::on_error(const rclcpp_lifecycle::State &)
{
  if (!reset())
  {
    return controller_interface::CallbackReturn::ERROR;
...
bool SkidDriveController::reset()
{
...
controller_interface::CallbackReturn SkidDriveController::on_shutdown(
  const rclcpp_lifecycle::State &)
{
  return controller_interface::CallbackReturn::SUCCESS;
}

void SkidDriveController::halt()
{
...
controller_interface::CallbackReturn SkidDriveController::configure_side(
...
CLASS_LOADER_REGISTER_CLASS(
  skid_drive_controller::SkidDriveController, controller_interface::ControllerInterface)
